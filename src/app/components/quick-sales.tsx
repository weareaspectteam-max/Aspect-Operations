...
import { Send, Clock, ShoppingCart, X, Plus, Trash2, Tag, XCircle, DollarSign, CheckCircle, ArrowLeft, AlertCircle } from 'lucide-react';
  ...
  const confirmCancelSale = () => {
    ...
    if (cancelSaleId) {
      ...
      if (confirm(`Bu satışı iptal etmek istediğinize emin misiniz?\\n\\nSebep: ${cancelReason}\\n\\nBu işlem geri alınamaz!`)) {
        // TODO: İptal sebebini kaydet (API'ye gönder)
        console.log('Satış iptal edildi:', cancelSaleId, 'Sebep:', cancelReason);
        setRecentSales(recentSales.filter(sale => sale.id !== cancelSaleId));
        ...
        <div>
          ...
          <div className="space-y-2">
            {recentSales.map((sale) => (
              <div
                key={sale.id}
                className="backdrop-blur-xl bg-gradient-to-br from-white/10 to-white/5 border border-white/20 rounded-xl p-4 hover:shadow-md transition-all"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#a8e6cf] to-[#8dd9b8] flex items-center justify-center text-[#2d3748] text-xl shadow-md">
                      ✓
                    </div>
                    <div>
